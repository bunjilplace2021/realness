...
// GLOBAL VARIABLES
import {Context, Noise, setContext, start, BiquadFilter, Meter, Oscillator, getContext} from 'tone';

import {changeMuteButton, changeTooltipText, changetoUnmute, hidePlayButton} from './UIFunctions';
// UTILITIES
import {
	probeLevel,
	debounce,
...
// switch logging
process.env.NODE_ENV === 'development' ? (window.logging = true) : (window.logging = false);
...
const showPromptTimer = setInterval(() => {
	...
	if (window.isMuted && !window.initinst && !window.instload_toggle) {
		console.log('showing sound prompt');
		window.soundVolume();
	}
}, 5000);
...
const UISound = () => {
	window.addEventListener('pixel_added', (e) => {
		...
		!window.isMuted && !u.isPlaying && u.play([~~e.detail.pixelX, window.height - ~~e.detail.pixelY]);
	});
};
			...
			try {
				...
				returnedBuffers.forEach(async (buf, i) => {
					try {
						...
						synths[i].grainOutput.gain.setValueAtTime(buf.idealGain, soundtrackAudioCtx.currentTime);
						synths[i].buffer.copyToChannel(returnedBuffers[i].getChannelData(0), 0, 0);
					} catch (error) {
...
const startRecording = async () => {
	return new Promise(async (resolve, reject) => {
		if (window.MediaRecorder && recordingAllowed && !window.isMuted) {
			try {
				window.audioUUIDs = window.audioUUIDs.filter((uuid) => uuid === f.audioUUID);
...
const stopRecording = async () => {
	if (r.recording && recordingAllowed) {
		...
		soundLog('User Audio UUID ' + window.audioUUIDs[window.audioUUIDs.length - 1]);
	...
	return new Promise(async (resolve, reject) => {
		...
		if (mp3Supported) {
			urls = await Promise.all(
				Array.from({length: numSources}, () => randomChoice(f.files.items).getDownloadURL())
			);
			urls.forEach((url) => {
			...
			urls.forEach((url) => bufPromises.push(aacDecode(url, soundtrackAudioCtx)));
		}
		let buffers;
	...
	return new Promise(async (resolve, _) => {
		...
		for (let i = 0; i < numSources; i++) {
			if (returnedBuffers[i]) {
				synths.push(new GrainSynth(returnedBuffers[i], soundtrackAudioCtx, numVoices));
				synths.forEach((synth) => {
					returnedBuffers[i].idealGain ? (synth.grainOutput.gain.value = returnedBuffers[i].idealGain) : null;
				});
		...
		if (ps.particles) {
			...
			if (Math.random() > 0.2) {
				synths.forEach((synth, i) => {
					...
					let filterFreq = Math.random() * mapValue(~~radius, 0, maxradius, 220, 2000);
					synth.filter.frequency.rampTo(filterFreq, 10);
				});
			}

			subOsc.filter.frequency.rampTo(mapValue(~~radius, 0, ~~maxradius, 50, 300), 10);
		}
	} catch (error) {
...
const debug = () => {
	...
	window.sourceCount = soundtrackAudioCtx.rawContext._nativeAudioContext.activeSourceCount;
