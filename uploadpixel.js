...
function shaderMousePressed() {
  ...
  console.time("pixelpg.get");
  let x = width - mouseX;
  let y = isSafari ? mouseY : height - mouseY;
  colour = pixelpg.get(x, y);

  console.timeEnd("pixelpg.get");
  ...
  if (webcam && !menu_loc) {
    console.time("pushing to firebase");
    test.push(data);
    console.log(data);
    console.timeEnd("pushing to firebase");
  }
}
