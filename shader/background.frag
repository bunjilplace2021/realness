...
st += pip_pos;
  ...
  float mousecirc = ComputeCircle(mousepos,vec2(0.5),radius,0.5);
  float mousecirc2 = ComputeCircle(mousepip,vec2(-10.,-10.),10.,0.5);

  vec3 mousecol = texture2D(tex1,vec2(u_mouse.x,u_mouse.y)).rgb;

  vec3 mousecol2 = vec3(1.0);

  vec3 pip = texture2D(tex1,st).rgb;
 ...
 vec3 colout = mix(bg, mousecol, 1.0 - mousecirc);

vec3 pip2 = mix(pip,mousecol2,1.0 - mousecirc2);

 vec3 outc = mix(colout,pip2,b);
